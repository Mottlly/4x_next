...
export default function GrassPatchLayer({ tiles, spacing, heightScale }) {
  return (
    <>
      {tiles
        .filter((tile) => tile.type === "plains" && tile.discovered) // <-- add tile.discovered
        .flatMap((tile) => {
          ...
          return positions.map(({ pos, rot }, i) => (
            <TallGrassPatch
              ...
              position={pos}
              scale={(0.5 + 0.18 * Math.sin(i * 1.7 + tile.q + tile.r)) * 1.1} // 3x bigger
              rotation={rot}
            />
          ));
        })}
