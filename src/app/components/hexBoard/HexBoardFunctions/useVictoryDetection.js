...
export default function useVictoryDetection(board, pieces, currentTurn) {
  ...
  const hostileFortresses = useMemo(() => {
    return (board.hostilePieces || []).filter(piece => piece.type === 'hostileFortress');
  }, [board.hostilePieces]);
  ...
  useEffect(() => {
    const settlementsBuilt = (board.tiles || []).filter(tile => 
      tile.building && ['reconstructed_shelter', 'resource_extractor', 'sensor_suite'].includes(tile.building)
    ).length;

    const totalResources = (board.resources || [0, 0, 0]).reduce((sum, resource) => sum + resource, 0);

    const hostileRaiders = (board.hostilePieces || []).filter(piece => piece.type === 'Raider').length;
  ...
  const performanceMetrics = useMemo(() => {
    const efficiency = currentTurn > 0 ? Math.max(1, Math.min(5, 6 - Math.floor(currentTurn / 10))) : 5;
