...
export const friendlyPiecesData = Object.entries(PIECE_BANK).map(
  ([key, piece]) => ({
    id: `friendly_${key}`,
    name: piece.type,
    ...
    upkeep: piece.upkeep
      ? `Rations: ${piece.upkeep.rations}, Materials: ${piece.upkeep.printingMaterial}, Weapons: ${piece.upkeep.weapons}`
      : "None",
    modelType: "piece",
...
export const hostilePiecesData = Object.entries(HOSTILE_PIECE_BANK).map(
  ([key, piece]) => ({
    id: `hostile_${key}`,
    name: piece.type,
    ...
    stats: {
      ...
      "Aggression Level": piece.aiBehavior?.aggressionLevel
        ? `${Math.round(piece.aiBehavior.aggressionLevel * 100)}%`
        : "N/A",
...
export const buildingsData = Object.entries(buildingOptions).map(
  ([key, building]) => ({
    id: `building_${key}`,
    name: building.label,
    category: "Structures",
...
export const terrainData = Object.entries(tileTypeStyles).map(
  ([key, tileStyle]) => ({
    id: `terrain_${key}`,
    ...
    stats: {
      ...
      Passable:
        key !== "impassable mountain" ? "Yes" : "Special abilities required",
    },
    modelType: "terrain",
...
function getFriendlyPieceDescription(type) {
  const descriptions = {
    ...
    Scout:
      "Fast reconnaissance unit with enhanced vision. Excellent for exploring unknown territories and spotting threats from a distance.",
    Engineer:
      "Versatile construction unit capable of building structures and extracting resources. Essential for colony expansion.",
    Security:
      "Heavily armed combat unit designed to protect your colony from hostile threats. High attack power but slower movement.",
    Armed_Settler:
      "Well-equipped colonist with basic combat training. Can establish settlements while defending themselves.",
  };
...
function getFriendlyPieceGameEffects(piece) {
  ...
  if (piece.vision > 2)
    effects.push("Extended vision range for reconnaissance");
  if (piece.abilities?.mountaineering)
    effects.push("Can traverse impassable mountains");
  if (piece.abilities?.seafaring) effects.push("Can cross water bodies");
  if (piece.abilities?.stealthy)
    effects.push("Moves undetected by some enemies");
...
function getHostilePieceDescription(type) {
  const descriptions = {
    Raider:
      "Aggressive human scavengers who attack settlements for resources. They move in groups and become more dangerous near their fortress bases.",
    hostileFortress:
      "Heavily fortified enemy stronghold that spawns raiders. These structures must be destroyed to achieve victory on this planet.",
  };
  return (
    descriptions[type] || "A dangerous hostile entity threatening your colony."
  );
}

function getHostilePieceGameEffects(piece) {
  const effects = [];
  if (piece.aiBehavior?.packTactics)
    effects.push("Attacks in coordinated groups");
  if (piece.aiBehavior?.ambush)
    effects.push("Uses stealth and surprise attacks");
  if (piece.aiBehavior?.aggressionLevel > 0.7)
    effects.push("Highly aggressive, attacks on sight");
  if (piece.aiBehavior?.patrolRadius > 3)
    effects.push("Patrols large areas around spawn point");
  if (piece.type === "hostileFortress")
    effects.push("Spawns raiders periodically; Must be destroyed for victory");
...
function getBuildingDescription(key) {
  const descriptions = {
    reconstructed_shelter:
      "Basic shelter constructed from salvaged pod materials. Provides protection and allows recruitment of new colonists.",
    resource_extractor:
...
function getBuildingGameEffects(key, building) {
  ...
  if (building.vision > 1)
    effects.push(`Provides vision range of ${building.vision} tiles`);
...
function getTerrainDescription(type) {
  const descriptions = {
    water:
      "Open water bodies that block movement for most units. Can be crossed by units with seafaring abilities.",
    lake: "Smaller water bodies with similar properties to open water but may offer strategic advantages.",
    plains:
      "Open grassland ideal for movement and basic resource extraction. Easy to traverse and build upon.",
    grassland:
      "Fertile land rich in food resources. Excellent for establishing resource extractors.",
    forest:
      "Dense woodland providing materials and cover. Slows movement but offers valuable printing materials.",
    mountain:
      "Rocky highlands rich in minerals and weapons materials. Difficult terrain but valuable for resource extraction.",
    "impassable mountain":
      "Extremely treacherous peaks that only specialized units can cross. Blocks most movement.",
  };
  return (
    descriptions[type] || "A terrain feature affecting movement and resources."
  );
}
...
function getTerrainGameEffects(type) {
  ...
  switch (type) {
    ...
    case "forest":
      effects.push(
        "Good printing material production",
        "Increased movement cost"
      );
      break;
    case "mountain":
      effects.push(
        "Excellent printing material and weapons production",
